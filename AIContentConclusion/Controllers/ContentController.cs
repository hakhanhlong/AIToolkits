...
using AIContentConclusion.ViewModels;

namespace AIContentConclusion.Controllers
{
    ...
    public class ContentController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("ContentConclusion")]
        public async Task<IActionResult> AnalyzeContentQuality([FromBody] ContentConclusionRequestViewModel request)
        {

            var promptFilePath = Path.Combine(Directory.GetCurrentDirectory(), "Prompts", "ContentConclusionVietnam", "skprompt.txt");
            var configFilePath = Path.Combine(Directory.GetCurrentDirectory(), "Prompts", "ContentConclusionVietnam", "config.json");
            ...
            {
                ResponseSchema = typeof(ContentConclusionResponseViewModel),
            ...
            })
            {
                ...
                ["tone"] = request.Tone,
                ["style"] = request.Style,
                ["conclusion_length"] = request.Conclusion_Length,
                ["number_of_conclusions"] = request.Number_Of_Conclusions,
            ...
            {
                Data = JsonConvert.DeserializeObject<ContentConclusionResponseViewModel>(responseData),
                Info = new
                {
